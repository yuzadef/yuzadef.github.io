...
One thing I have not yet mentioned above, in order for the Open Redirect or XSS to trigger, the victims would have to click on "Continue" button at the bottom of the page. Also, if the victims are authenticated, navigating to the vulnerable page will immediately redirect them to `/dashboard`. So, our exploitation surface is a bit limited currently as we can only exploit unauthenticated users and the payload triggers on one-click. Well, at least it's not a Self-XSS
